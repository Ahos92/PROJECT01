	...
	public boolean saveDailyAmount() {
		...
		try {
			...
			if (rs.next()) {
				...
				conn.rollback();
				System.err.println("정상적인 처리가 되지 않았습니다.(sql01) 본사에 연락해주세요. 02-000-0000");
			}
			...
			try {
				...
				if(result != 1){
					conn.rollback();
					System.err.println("정상적인 처리가 되지 않았습니다.(sql02) 본사에 연락해주세요. 02-000-0000");
				} else {
//					conn.commit();
				...
				System.err.println("정상적인 처리가 되지 않았습니다.(PK) 본사에 연락해주세요. 02-000-0000");
			}
